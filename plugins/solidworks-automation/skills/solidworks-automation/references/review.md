...
report, report_path = run_review(
    model,
    r"C:\temp\mcp__solidworks__solidworks_review",
    basename="model",
...
任务中明确给出外形或孔径，但交付物没有独立 JSON 证据时，Reviewer Gate 必须失败；DSH 的文字总结和“执行器自报通过”不能替代证据。孔数量、孔位、边距、中心距、圆角、壁厚和装配自由度也应按任务逐项记录。
...
python scripts/sw_review.py ^
  ...
  --output-dir C:\temp\mcp__solidworks__solidworks_review ^
  --basename model ^
