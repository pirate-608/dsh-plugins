...
description: "SolidWorks CAD 自动化技能，可通过 Python COM 接口与 OpenClaw / DSH / Claude 协作控制 Windows 上运行的 SolidWorks，用于零件建模、装配体、工程图、钣金、焊件、仿真、文件导出、自定义属性、设计表、配置管理，以及 OBJ/STL/STEP 等外来模型导入；当用户提到 SolidWorks、SW、OpenClaw、龙虾、3D 建模、CAD、零件、装配、工程图、钣金、焊件、导出 STEP/STL/PDF、BOM、设计表、公开三维模型、网格参考模型、原版外观复刻或高还原产品外观时使用。"
---
...
当用户要求“让 SolidWorks 支持 MCP”“接入 DSH/Claude Desktop 工具调用”“不要每次生成一大段 Python 脚本”时：

1. 读取 `references/mcp-server.md`。
2. DSH 插件通过受管 Preset 启动本地 `stdio` server；手动诊断时运行 `python mcp-server/server.py`。
3. 工具调用优先走 `mcp__solidworks__solidworks_health_check`、`mcp__solidworks__solidworks_create_basic_part`、`mcp__solidworks__solidworks_create_hole_feature`、`mcp__solidworks__solidworks_inspect_hole_features`、`mcp__solidworks__solidworks_add_component`、`mcp__solidworks__solidworks_add_coincident_mate`、`mcp__solidworks__solidworks_add_distance_mate`、`mcp__solidworks__solidworks_add_concentric_mate`、`mcp__solidworks__solidworks_set_component_fixed`、`mcp__solidworks__solidworks_export_active`、`mcp__solidworks__solidworks_review_active`、`mcp__solidworks__solidworks_add_rotary_motor`、`mcp__solidworks__solidworks_inspect_motion_studies`、`mcp__solidworks__solidworks_validate_motion_study`。
...
7. 计算后调用 `sw_motion.validate_motion_studies()` 或 MCP `mcp__solidworks__solidworks_validate_motion_study`，检查类型、时长、马达数量、结果存在性和 `results_out_of_date=False`；只看到时间轴、动画或 `Calculate=True` 不算验收通过。
