...
description: "AutoCAD 自动化技能。用于让 DSH/Claude 在 Windows 上通过 Python COM/ActiveX 直连本机 AutoCAD 进行二维绘图、图层/文字/标注/块处理、DWG/DXF/PDF 导出、批量修改、工程图自检，以及在 COM 不足时切换到 AutoLISP/SCR 或 AutoCAD .NET 插件路线；当用户提到 AutoCAD、CAD、DWG、DXF、画图、出图、图层、标注、块、批量改图、AutoLISP、ObjectARX、.NET 插件、DSH 接入 CAD 软件时都应优先使用。"
---
...
- 让 DSH 直接连接本机 AutoCAD、保存 DWG、导出 DXF/PDF，并生成复核报告。
...
| 需求 | 首选路线 | 说明 |
|---|---|---|
| 快速绘制、改图、批量检查 | Python COM/ActiveX | 本技能默认路线，可由 DSH 直接通过 shell 执行 |
